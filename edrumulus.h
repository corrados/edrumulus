...
public:
  ...
  int  get_midi_note           ( const int pad_idx ) { return
    rim_state[pad_idx] == Pad::RIM_SHOT       ? pad[pad_idx].get_midi_note_rim() :
    rim_state[pad_idx] == Pad::RIM_ONLY       ? pad[pad_idx].get_midi_note_open_rim() :
    rim_state[pad_idx] == Pad::RIM_SIDE_STICK ? pad[pad_idx].get_midi_note_open() : pad[pad_idx].get_midi_note(); }
  ...
  // configure the pads
  void set_pad_type             ( const int pad_idx, const Pad::Epadtype new_pad_type ) { set_coupled_pad_idx ( pad_idx, 0 /* disable possible previous coupling first */ ); pad[pad_idx].set_pad_type ( new_pad_type ); }
